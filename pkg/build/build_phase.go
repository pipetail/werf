...
func (phase *BuildPhase) onImageStage(ctx context.Context, img *Image, stg stage.Interface, isEmpty bool) error {
	...
	if phase.ShouldBeBuiltMode {
		...
		defer phase.Conveyor.GetStageDependenciesDigestMutex(stg.GetDependenciesDigest()).Unlock()
		return nil
	} else {
...
func (phase *BuildPhase) calculateStage(ctx context.Context, img *Image, stg stage.Interface, shouldBeBuiltMode bool) error {
	...
	stageSig, err := calculateDependenciesDigest(ctx, string(stg.Name()), stageDependencies, phase.StagesIterator.PrevNonEmptyStage, phase.Conveyor)
	if err != nil {
		return err
	}
	stg.SetDependenciesDigest(stageSig)
	...
	logboek.Context(ctx).Info().LogProcessInline("Locking stage %s handling", stg.LogDetailedName()).
		...
		Do(phase.Conveyor.GetStageDependenciesDigestMutex(stg.GetDependenciesDigest()).Lock)

	if stages, err := phase.Conveyor.StorageManager.GetStagesByDependenciesDigest(ctx, stg.LogDetailedName(), stageSig); err != nil {
		return err
	} else {
	...
	stageContentSig, err := calculateDependenciesDigest(ctx, fmt.Sprintf("%s-content", stg.Name()), "", stg, phase.Conveyor)
	if err != nil {
...
func (phase *BuildPhase) prepareStageInstructions(ctx context.Context, img *Image, stg stage.Interface) error {
	...
	serviceLabels := map[string]string{
		...
		imagePkg.WerfStageDependenciesDigestLabel:        stg.GetDependenciesDigest(),
...
func (phase *BuildPhase) atomicBuildStageImage(ctx context.Context, img *Image, stg stage.Interface) error {
	...
	if v := os.Getenv("WERF_TEST_ATOMIC_STAGE_BUILD__SLEEP_SECONDS_BEFORE_STAGE_BUILD"); v != "" {
		...
		fmt.Printf("Sleeping %d seconds before building new image by dependencies-digest %s...\n", seconds, stg.GetDependenciesDigest())
	...
	if err := logboek.Context(ctx).Streams().DoErrorWithTag(fmt.Sprintf("%s/%s", img.LogName(), stg.Name()), img.LogTagStyle(), func() error {
		...
		return fmt.Errorf("failed to build image for stage %s with dependencies-digest %s: %s", stg.Name(), stg.GetDependenciesDigest(), err)
	}

	if v := os.Getenv("WERF_TEST_ATOMIC_STAGE_BUILD__SLEEP_SECONDS_BEFORE_STAGE_SAVE"); v != "" {
		seconds := 0
		fmt.Sscanf(v, "%d", &seconds)
		fmt.Printf("Sleeping %d seconds before saving newly built image %s into stages storage %s by dependencies-digest %s...\n", seconds, stg.GetImage().GetBuiltId(), phase.Conveyor.StorageManager.StagesStorage.String(), stg.GetDependenciesDigest())
		time.Sleep(time.Duration(seconds) * time.Second)
	}

	if lock, err := phase.Conveyor.StorageLockManager.LockStage(ctx, phase.Conveyor.projectName(), stg.GetDependenciesDigest()); err != nil {
		return fmt.Errorf("unable to lock project %s dependencies-digest %s: %s", phase.Conveyor.projectName(), stg.GetDependenciesDigest(), err)
	} else {
		defer phase.Conveyor.StorageLockManager.Unlock(ctx, lock)
	}

	if stages, err := phase.Conveyor.StorageManager.GetStagesByDependenciesDigest(ctx, stg.LogDetailedName(), stg.GetDependenciesDigest()); err != nil {
		return err
	} else {
		if stageDesc, err := phase.Conveyor.StorageManager.SelectSuitableStage(ctx, phase.Conveyor, stg, stages); err != nil {
			return err
			...
			logboek.Context(ctx).Default().LogF(
				"Discarding newly built image for stage %s by dependencies-digest %s: detected already existing image %s in the stages storage\n",
				stg.LogDetailedName(), stg.GetDependenciesDigest(), stageDesc.Info.Name,
			)
			...
			newStageImageName, uniqueID := phase.Conveyor.StorageManager.GenerateStageUniqueID(stg.GetDependenciesDigest(), stages)
			...
			stageImageObj.GetStageDescription().StageID = &image.StageID{DependenciesDigest: stg.GetDependenciesDigest(), UniqueID: uniqueID}
			...
			if err := logboek.Context(ctx).Default().LogProcess("Store into stages storage").DoError(func() error {
				if err := phase.Conveyor.StorageManager.StagesStorage.StoreImage(ctx, &container_runtime.DockerImage{Image: stageImage}); err != nil {
					return fmt.Errorf("unable to store stage %s dependencies-digest %s image %s into stages storage %s: %s", stg.LogDetailedName(), stg.GetDependenciesDigest(), stageImage.Name(), phase.Conveyor.StorageManager.StagesStorage.String(), err)
				}
				return nil
			}); err != nil {
				return err
			...
			return phase.Conveyor.StorageManager.AtomicStoreStagesByDependenciesDigestToCache(ctx, string(stg.Name()), stg.GetDependenciesDigest(), stageIDs)
		}
	}
}
...
func calculateDependenciesDigest(ctx context.Context, stageName, stageDependencies string, prevNonEmptyStage stage.Interface, conveyor *Conveyor) (string, error) {
	...
	if prevNonEmptyStage != nil {
		...
		checksumArgs = append(checksumArgs, prevNonEmptyStage.GetDependenciesDigest(), prevStageDependencies)
	}

	dependenciesDigest := util.Sha3_224Hash(checksumArgs...)

	blockMsg := fmt.Sprintf("Stage %s dependencies-digest %s", stageName, dependenciesDigest)
	logboek.Context(ctx).Debug().LogBlock(blockMsg).Do(func() {
		checksumArgsNames := []string{
			"BuildCacheVersion",
			"stageName",
			"stageDependencies",
			"prevNonEmptyStage dependenciesDigest",
...
func (phase *BuildPhase) printShouldBeBuiltError(ctx context.Context, img *Image, stg stage.Interface) {
	logboek.Context(ctx).Default().LogProcess("Built stages cache check").
		...
		Do(func() {
			logboek.Context(ctx).Warn().LogF("%s with dependencies-digest %s is not exist in stages storage\n", stg.LogDetailedName(), stg.GetDependenciesDigest())
			...
			if img.isDockerfileImage {
				logboek.Context(ctx).Warn().LogLn(reasonNumberFunc() + `Dockerfile has COPY or ADD instruction which uses non-permanent data that affects stage dependenciesDigest:
...
- auto-generated file`)
			...
			logboek.Context(ctx).Warn().LogLn(reasonNumberFunc() + `werf.yaml has non-permanent data that affects stage dependenciesDigest:
...
- auto-generated file content (e.g. {{ .Files.Get "hash_sum_of_something" }})`)
			...
			logboek.Context(ctx).Warn().LogLn(`Stage dependenciesDigest dependencies can be found here, https://werf.io/documentation/reference/stages_and_images.html#stage-dependencies.
