...
func (i *Image) SetDigest(sig string) {
	i.digest = sig
}
...
func (i *Image) GetImageInfoGetter() *image.InfoGetter {
	info := i.GetLastNonEmptyStage().GetImage().GetStageDescription().Info
...
func (i *Image) FetchBaseImage(ctx context.Context, c *Conveyor) error {
	...
	case StageAsBaseImage:
		...
		if err := c.StorageManager.FetchStage(ctx, i.stageAsBaseImage); err != nil {
			return err
		}
	default:
