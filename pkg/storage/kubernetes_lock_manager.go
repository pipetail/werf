...
func (manager *KuberntesLockManager) LockStage(ctx context.Context, projectName, dependenciesDigest string) (LockHandle, error) {
	if locker, err := manager.getLockerForProject(ctx, projectName); err != nil {
		return LockHandle{}, err
	} else {
		_, lock, err := locker.Acquire(kubernetesStageLockName(projectName, dependenciesDigest), werf.SetupLockerDefaultOptions(ctx, lockgate.AcquireOptions{}))
		return LockHandle{LockgateHandle: lock, ProjectName: projectName}, err
	}
}

func (manager *KuberntesLockManager) LockStageCache(ctx context.Context, projectName, dependenciesDigest string) (LockHandle, error) {
	if locker, err := manager.getLockerForProject(ctx, projectName); err != nil {
		return LockHandle{}, err
	} else {
		_, lock, err := locker.Acquire(kubernetesStageCacheLockName(projectName, dependenciesDigest), werf.SetupLockerDefaultOptions(ctx, lockgate.AcquireOptions{}))
		return LockHandle{LockgateHandle: lock, ProjectName: projectName}, err
	}
}
...
func kubernetesStageLockName(_, dependenciesDigest string) string {
	return fmt.Sprintf("stage/%s", dependenciesDigest)
}

func kubernetesStageCacheLockName(_, dependenciesDigest string) string {
	return fmt.Sprintf("stage-cache/%s", dependenciesDigest)
}
