...
func (m *StagesStorageManager) ForEachDeleteStage(ctx context.Context, options ForEachDeleteStageOptions, stagesDescriptions []*image.StageDescription, f func(ctx context.Context, stageDesc *image.StageDescription, err error) error) error {
	...
	for _, stageDesc := range stagesDescriptions {
		if err := m.StagesStorageCache.DeleteStagesByDependenciesDigest(ctx, m.ProjectName, stageDesc.StageID.DependenciesDigest); err != nil {
			return fmt.Errorf("unable to delete stages storage cache record (%s): %s", stageDesc.StageID.DependenciesDigest, err)
		}
	}
...
func (m *StagesStorageManager) FetchStage(ctx context.Context, stg stage.Interface) error {
	...
	if freshStageDescription, err := m.StagesStorage.GetStageDescription(ctx, m.ProjectName, stg.GetImage().GetStageDescription().StageID.DependenciesDigest, stg.GetImage().GetStageDescription().StageID.UniqueID); err != nil {
		return err
...
func (m *StagesStorageManager) SelectSuitableStage(ctx context.Context, c stage.Conveyor, stg stage.Interface, stages []*image.StageDescription) (*image.StageDescription, error) {
	...
	if err := logboek.Context(ctx).Info().LogProcess("Selecting suitable image for stage %s by dependencies-digest %s", stg.Name(), stg.GetDependenciesDigest()).
		DoError(func() error {
			var err error
...
func (m *StagesStorageManager) AtomicStoreStagesByDependenciesDigestToCache(ctx context.Context, stageName, stageSig string, stageIDs []image.StageID) error {
	if lock, err := m.StorageLockManager.LockStageCache(ctx, m.ProjectName, stageSig); err != nil {
		return fmt.Errorf("error locking stage %s cache by dependencies-digest %s: %s", stageName, stageSig, err)
	} else {
		defer m.StorageLockManager.Unlock(ctx, lock)
	}

	return logboek.Context(ctx).Info().LogProcess("Storing stage %s images by dependencies-digest %s into stages storage cache", stageName, stageSig).
		DoError(func() error {
			if err := m.StagesStorageCache.StoreStagesByDependenciesDigest(ctx, m.ProjectName, stageSig, stageIDs); err != nil {
				return fmt.Errorf("error storing stage %s images by dependencies-digest %s into stages storage cache: %s", stageName, stageSig, err)
			}
			return nil
		})
}

func (m *StagesStorageManager) GetStagesByDependenciesDigest(ctx context.Context, stageName, stageSig string) ([]*image.StageDescription, error) {
	cacheExists, cacheStages, err := m.getStagesByDependenciesDigestFromCache(ctx, stageName, stageSig)
	if err != nil {
		return nil, err
	}
	if cacheExists {
	...
	logboek.Context(ctx).Info().LogF(
		"Stage %s cache by dependencies-digest %s is not exists in the stages storage cache: resetting stages storage cache\n",
		stageName, stageSig,
	)
	return m.atomicGetStagesByDependenciesDigestWithCacheReset(ctx, stageName, stageSig)
}

func (m *StagesStorageManager) getStagesByDependenciesDigestFromCache(ctx context.Context, stageName, stageSig string) (bool, []*image.StageDescription, error) {
	var cacheExists bool
	var cacheStagesIDs []image.StageID

	err := logboek.Context(ctx).Info().LogProcess("Getting stage %s images by dependencies-digest %s from stages storage cache", stageName, stageSig).
		DoError(func() error {
			var err error
			cacheExists, cacheStagesIDs, err = m.StagesStorageCache.GetStagesByDependenciesDigest(ctx, m.ProjectName, stageSig)
			if err != nil {
...
func (m *StagesStorageManager) atomicGetStagesByDependenciesDigestWithCacheReset(ctx context.Context, stageName, stageSig string) ([]*image.StageDescription, error) {
	if lock, err := m.StorageLockManager.LockStageCache(ctx, m.ProjectName, stageSig); err != nil {
	...
	if err := logboek.Context(ctx).Default().LogProcess("Get %s stages by dependencies-digest %s from stages storage", stageName, stageSig).
		DoError(func() error {
			var err error
			stageIDs, err = m.StagesStorage.GetStagesIDsByDependenciesDigest(ctx, m.ProjectName, stageSig)
			if err != nil {
	...
	if err := logboek.Context(ctx).Info().LogProcess("Storing %s stages by dependencies-digest %s into stages storage cache", stageName, stageSig).
		DoError(func() error {
			if err := m.StagesStorageCache.StoreStagesByDependenciesDigest(ctx, m.ProjectName, stageSig, stageIDs); err != nil {
				return fmt.Errorf("error storing stage %s images by dependencies-digest %s into stages storage cache: %s", stageName, stageSig, err)
			}
...
func (m *StagesStorageManager) getStageDescription(ctx context.Context, stageID image.StageID, opts getStageDescriptionOptions) (*image.StageDescription, error) {
	stageImageName := m.StagesStorage.ConstructStageImageName(m.ProjectName, stageID.DependenciesDigest, stageID.UniqueID)
	...
	if opts.WithManifestCache {
		...
		if imgInfo, err := image.CommonManifestCache.GetImageInfo(ctx, m.StagesStorage.String(), stageImageName); err != nil {
			...
			return &image.StageDescription{
				StageID: &image.StageID{DependenciesDigest: stageID.DependenciesDigest, UniqueID: stageID.UniqueID},
				Info:    imgInfo,
			}, nil
		} else {
	...
	logboek.Context(ctx).Debug().LogF("Getting dependenciesDigest %q uniqueID %d stage info from %s...\n", stageID.DependenciesDigest, stageID.UniqueID, m.StagesStorage.String())
	if stageDesc, err := m.StagesStorage.GetStageDescription(ctx, m.ProjectName, stageID.DependenciesDigest, stageID.UniqueID); err != nil {
		return nil, fmt.Errorf("error getting dependenciesDigest %q uniqueID %d stage info from %s: %s", stageID.DependenciesDigest, stageID.UniqueID, m.StagesStorage.String(), err)
	} else if stageDesc != nil {
...
func (m *StagesStorageManager) GenerateStageUniqueID(dependenciesDigest string, stages []*image.StageDescription) (string, int64) {
	var imageName string

	for {
		timeNow := time.Now().UTC()
		uniqueID := timeNow.Unix()*1000 + int64(timeNow.Nanosecond()/1000000)
		imageName = m.StagesStorage.ConstructStageImageName(m.ProjectName, dependenciesDigest, uniqueID)
